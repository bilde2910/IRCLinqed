...
public class Packet322List extends NumericResponsePacket {
	public Packet322List(IRCLinqed il, IRCThread thread, String command) {
		super(il, thread, command);
	}

	@Override
	public void handle() {
		this.thread.addGuiListItem(new ListItem(getParam(1), getParam(2) + " users visible", getMessage()));
	}
}
