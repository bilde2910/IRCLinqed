...
import info.varden.irclinqed.packet.Packet315EndOfWho;
...
public class GuiScreenWhoReply extends GuiScreenWithList {
	private GuiItemList itemList;
	private Packet315EndOfWho packet;
	public GuiSmallButton pmButton;
	
	public GuiScreenWhoReply(Packet315EndOfWho packet) {
		this.packet = packet;
	}
	...
	public void actionPerformed(GuiButton button) {
		if (button.enabled) {
			if (button.id == 5) {
				Minecraft.getMinecraft().displayGuiScreen(new GuiChat(IRCLinqed.instance.configLoader.getKey("chatchar", "#") + " /msg " + itemList.getSelectedItem().title.split(" ")[0] + " "));
			} else if (button.id == 6) {
	...
	public void drawScreen(int par1, int par2, float par3) {
        ...
        this.drawCenteredString(this.fontRenderer, this.itemList.getSize() + " results for /who " + this.packet.getParam(1), this.width / 2, 16, 16777215);
