...
public class IngameIPSetter implements IIPFetcher {
	
	private final IRCLinqed il;
	
	public IngameIPSetter(IRCLinqed il) {
		this.il = il;
	}
	...
	public void onIPFound(String ip) {
		...
		if (ip != null) {
			...
			this.il.util.writeToChat(MessageType.LQS_INFO, "IP address found: " + ip);
		} else {
			this.il.logSevere("Could not obtain IP address! Internet DCC connections will not work!");
			this.il.util.writeToChat(MessageType.LQS_ERROR, "Could not obtain IP address!");
		}
	}
