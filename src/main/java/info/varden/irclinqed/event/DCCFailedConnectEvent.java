...
public class DCCFailedConnectEvent implements Event {
	private DCCConnectionHelper data;
	private Exception ex;
	
	public DCCFailedConnectEvent(DCCConnectionHelper data, Exception ex) {
		this.data = data;
		this.ex = ex;
	}
	...
	public void handle(EventHandler handler) {
		if (handler instanceof IDCCConnectionHandler) {
			((IDCCConnectionHandler) handler).onDCCFailedConnect(this.data, this.ex);
		}
	}
